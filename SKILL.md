...
7. skill 可以长期持续改进，但每一次 agent 优化任务必须有明确目标、评估标准和退出条件。
...
- 默认先跑一轮 `suite -> judge`；只有发现真实问题时才开启下一轮优化。
- 自动循环必须使用有限 `--max-rounds`，不要让单次 agent 任务无界运行。
...
- 不要把长期持续改进实现为单次任务里的无限循环；每轮都要可验证、可停止、可审计。
